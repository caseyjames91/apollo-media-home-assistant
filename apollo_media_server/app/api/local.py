...
def create_mapping(payload: PathMappingCreate, db: Session = Depends(get_db)):
    row = PathMapping(**payload.model_dump()); db.add(row); db.commit(); db.refresh(row); return row
...
def upsert_local_source(payload: LocalSourceUpsert, db: Session = Depends(get_db)):
    ...
    q = select(LocalAvailability).where(LocalAvailability.media_id == payload.media_id,
        LocalAvailability.provider == payload.provider, LocalAvailability.provider_item_id == payload.provider_item_id)
    row = db.scalar(q)
    values = dict(provider=payload.provider, provider_item_id=payload.provider_item_id, source_path=payload.source_path,
                  kodi_path=kodi_path or "", available=bool(kodi_path), quality=payload.quality, updated_at=datetime.now(timezone.utc))
    if row is None:
        row = LocalAvailability(media_id=payload.media_id, **values); db.add(row)
    else:
        for k,v in values.items(): setattr(row,k,v)
    db.commit(); db.refresh(row)
    return {"media_id": str(payload.media_id), "available_locally": row.available, "kodi_path": row.kodi_path or None}
