...
public class IrBridgeService extends Service {
    ...
    private void createChannel() {
        if (Build.VERSION.SDK_INT >= 26) {
            NotificationChannel ch = new NotificationChannel(CHANNEL_ID, "AVA IR Bridge", NotificationManager.IMPORTANCE_LOW);
    ...
    private void handle(Socket socket) {
        try (Socket s = socket;
            ...
            if ("GET".equals(method) && "/status".equals(path)) {
                ...
                body = "{\"ok\":true,\"emitter\":" + (ir != null && ir.hasIrEmitter()) + ",\"port\":" + PORT + ",\"broadlink\":true}";
            } else if ("POST".equals(method) && "/ir/broadlink".equals(path)) {
                try {
                    if (requestBody.isEmpty()) throw new IllegalArgumentException("Request body must be a BroadLink Base64 IR code");
                    int carrier = queryInt(rawPath, "carrier", DEFAULT_CARRIER_HZ);
                    int[] pattern = transmitBroadlink(this, requestBody, carrier);
                    body = "{\"ok\":true,\"format\":\"broadlink_base64\",\"carrier_hz\":" + carrier + ",\"pulses\":" + pattern.length + "}";
                } catch (Exception e) {
                    code = 400;
                    body = errorJson(e);
                }
            ...
            String reason = code == 200 ? "OK" : code == 404 ? "Not Found" : code == 400 ? "Bad Request" : "Error";
    ...
    public static int[] transmitBroadlink(Context context, String base64Code, int carrierHz) {
        ConsumerIrManager ir = requireIr(context);
        int[] pattern = decodeBroadlinkIr(base64Code);
        if (pattern.length < 2) throw new IllegalArgumentException("BroadLink code contained no usable IR pulse data");
    ...
    private static ConsumerIrManager requireIr(Context context) {
        ...
        if (ir == null || !ir.hasIrEmitter()) throw new IllegalStateException("No consumer IR emitter");
        return ir;
    }

    /**
     * Decode a BroadLink learned IR packet (Base64, packet type 0x26) to Android
     * ConsumerIrManager mark/space durations in microseconds.
     *
     * BroadLink stores duration ticks as microseconds * 269 / 8192. Values >=256
     * use a leading zero followed by a 16-bit big-endian value. A very long gap
     * marks the end of the useful learned frame, so we stop there rather than
     * replaying accidental capture repeats.
     */
    public static int[] decodeBroadlinkIr(String base64Code) {
        ...
        if (packet.length < 6) throw new IllegalArgumentException("BroadLink packet too short");
        if ((packet[0] & 0xff) != 0x26) throw new IllegalArgumentException("Not a BroadLink IR packet (type 0x26)");
        ...
        while (i < end) {
            ...
            // Learned BroadLink packets often contain a ~100 ms separator followed
            // by duplicate frames. One clean frame is enough for ConsumerIrManager.
    ...
    private static String errorJson(Exception e) {
        return "{\"ok\":false,\"error\":\"" + jsonEscape(e.toString()) + "\"}";
    }
