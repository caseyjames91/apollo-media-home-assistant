...
class AmsAuthority0983Tests(unittest.TestCase):
    def test_kodi_gui_and_headless_consume_ams_first(self):
        ...
        self.assertIn("rows = ams_continue_watching_rows()", MAIN)
        self.assertNotIn("sync_jellyfin", MAIN)
        self.assertNotIn("jellyfin/sync", MAIN)
    ...
    def test_card_artwork_is_ams_metadata_backed(self):
        self.assertIn("item?.poster_url", CARD)
        self.assertIn("item?.backdrop_url", CARD)
        self.assertIn("poster: posterUrl", CARD)
        self.assertIn("fanart: backdropUrl", CARD)
        self.assertNotIn("amsArtworkBlobUrl", CARD)
        self.assertNotIn("hydrateAmsArtwork", CARD)
        self.assertNotIn("jellyfin/image", CARD)
