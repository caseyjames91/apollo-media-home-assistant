...
def discovery_show(addon, tmdb_id):
    ...
    if not tmdb_id:
        raise RuntimeError("Discovery show requires a TMDB identity")
...
def discovery_season(addon, tmdb_id, season):
    tmdb_id = str(tmdb_id or "").strip()
    if not tmdb_id:
        raise RuntimeError("Discovery season requires a TMDB identity")
    result = request(
        addon,
        f"discovery/show/{tmdb_id}/season/{int(season)}",
        timeout=20,
    ) or {}
    return result if isinstance(result, dict) else {}
